...
- ISBNシリーズ供給源診断のProvider設定状態を実際のAPI有効状態と一致させ、未設定楽天Books／既定OFF Google Booksを通信対象から除外
- ISBN検索のAPI実測計測を修正
...
v4.13.105では、ISBN解決時に要求ISBNと一致しないProvider書誌を代用しないよう修正し、複数冊一括登録で別ISBNの書誌が混入して冊数が減る経路を遮断しました。あわせて一括登録の選択冊数・登録候補・書誌準備結果・保存前後の蔵書件数とISBNを診断表示します。Google Booksは既定OFF、楽天Booksは認証情報が設定されている場合のみ候補です。
