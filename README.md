...
v4.13.104では、検索結果からの一括登録時に既存の不完全な蔵書を誤って「登録済み」として除外する経路を修正しました。選択した検索結果はISBN書誌解決で不足情報を補完し、同一ISBNの旧ブランクレコードを置換します。あわせてNDL OpenSearchの名前空間付きDC/DC-NDL項目の正規化を修正しました。Google Booksは既定OFF、楽天Booksは認証情報が設定されている場合のみ候補です。
