...
## v4.13.9
- v4.13.8の最終UIを維持したまま、文字サイズ「小／中／大」の適用漏れを見直し。
- テーマ選択、蔵書の「詳細を見る」、読了／未読・お気に入り等のステータス、選択操作、検索・カレンダー・詳細画面などの固定フォントサイズを文字サイズ設定に追従させた。
- 大きい文字でボタンや固定幅ステータスがはみ出さないよう、必要箇所のみ折り返しを許可。
- 中サイズ時の既存UIは変更しない方針。
