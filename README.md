...
v4.13.91では、無課金の通常キーワード検索をNDL SearchのSRUへ切り替え、図書（`mediatype=books`）に限定しました。`dcndl` / `onlyBib=true` を指定してシリーズ名・巻数・ISBNを取得し、楽譜などの非図書資料が通常検索結果へ混入しにくい構成へ変更しています。検索結果カードはAPIから得た巻数も表示します。Google Booksは既定OFF、楽天Booksは認証情報が設定されている場合のみ候補です。
