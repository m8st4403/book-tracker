...
NDL SearchのOpenSearch Adapterは実装・fixtureテスト済みですが、iPhoneのブラウザからの直結通信を既定では無効化しています。Google Books障害時に未検証のNDL直結で追加待ち時間が発生しないようにし、将来バックエンド／プロキシ等の安全な通信経路を用意した時点で再有効化します。v4.13.75ではGoogle BooksのHTTP 429を確認した結果を反映し、Retry-Afterを取得・上限1秒で再試行し、再試行待ち時間を明示計測します。長い指数バックオフがProviderタイムアウトに隠れないようにし、429が続く場合はHTTP 429として即時に失敗します。
