...
NDL SearchのOpenSearch Adapterは実装・fixtureテスト済みですが、iPhoneのブラウザからの直結通信を既定では無効化しています。Google Books障害時に未検証のNDL直結で追加待ち時間が発生しないようにし、将来バックエンド／プロキシ等の安全な通信経路を用意した時点で再有効化します。v4.13.73ではProvider内の応答・JSON開始・JSON工程を明示計測し、iOS/WebKitでAbortイベントが遅れる場合にも短周期監視で本文処理を中断できるよう強化しました。
