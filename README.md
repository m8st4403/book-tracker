...
NDL SearchのOpenSearch Adapterは実装・fixtureテスト済みですが、iPhoneのブラウザからの直結通信を既定では無効化しています。Google Books障害時に未検証のNDL直結で追加待ち時間が発生しないようにし、将来バックエンド／プロキシ等の安全な通信経路を用意した時点で再有効化します。v4.13.78ではGoogle Books AdapterからHTTP 429再試行用のタイムアウト延長コールバックをgetJSONへ確実に伝播させ、実通信経路でも再試行延長が機能するよう修正しました。
