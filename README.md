...
v4.13.101では、NDL SearchのSRU検索条件を現行仕様に合わせ、`dpid=iss-ndl-opac` + `mediatype=books` で図書書誌を取得します。`dcndl` / `onlyBib=true` を指定してシリーズ名・巻数・ISBNを取得し、楽譜などの非図書資料が通常検索結果へ混入しにくい構成にしています。検索結果カードはAPIから得た巻数も表示します。Google Booksは既定OFF、楽天Booksは認証情報が設定されている場合のみ候補です。
