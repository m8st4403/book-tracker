...
async function main(){
  ...
  try {
    ...
    const searchRegistrationRepairContract=await evalJS(`(()=>{try{const oldBooks=books.slice();const existing={isbn:'9784088720715',title:'レベルE',author:'',publisher:'',date:'',series:null};const incoming={isbn:'9784088720715',title:'レベルE 1',author:'冨樫義博',publisher:'集英社',date:'1996',series:{name:'レベルE',volumeNumber:1}};books=[existing];const replace=shouldReplaceOwnedRecord(existing,incoming);const merged=mergeSearchResultIntoOwned(existing,incoming);const searchHtml=card(incoming,true,'result',0);books=oldBooks;return {ok:replace&&merged.author==='冨樫義博'&&merged.publisher==='集英社'&&merged.series?.volumeNumber===1&&searchHtml.includes('冨樫義博')&&!searchHtml.includes('ISBN')?false:replace&&merged.author==='冨樫義博'&&merged.publisher==='集英社'&&merged.series?.volumeNumber===1&&searchHtml.includes('冨樫義博'),replace,scoreIncoming:bookInfoScore(incoming),scoreExisting:bookInfoScore(existing)}}catch(e){return {ok:false,error:String(e?.message||e)}}})()`);
    check('E2E-REG-007 検索結果が不完全な既存蔵書を修復',searchRegistrationRepairContract?.ok===true,JSON.stringify(searchRegistrationRepairContract));
