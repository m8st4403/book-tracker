#!/usr/bin/env node
/* Book Tracker v4.11.2 static development guard. Usage: node dev_guard.js index.html */
const fs=require('fs');
...
add('APP_VERSION 4.11.2',/const\s+APP_VERSION\s*=\s*["']4\.11\.2["']/.test(s),'OK');
...
add('runtime regression tests',/REG-001: filter reset/.test(s)&&/REG-002: sort/.test(s)&&/REG-003: titles ending in bare numbers/.test(s)&&/REG-004: compact library stats/.test(s)&&/REG-005: long titles\/series headers/.test(s),'OK');
