...
check('STATIC-046 visible appVersionText uses current release', html.includes('id="appVersionText">バージョン：4.13.136') && !html.includes('id="appVersionText">バージョン：4.13.106'), 'settings-visible appVersionText matches current release');check('STATIC-019 persistence/backup gap audit exists', fs.existsSync(path.join(path.dirname(target),'RULE_GAP_AUDIT_v4_13_44.md'))&&fs.existsSync(path.join(path.dirname(target),'NEXT_IMPLEMENTATION_PRIORITY_v4_13_44.md')), 'persistence/backup/priority contracts');
