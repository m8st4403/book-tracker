...
check('STATIC-038 HTTP 429 header diagnostic', html.includes('429-Retry-After=') && html.includes('retry-429-Retry-After='), '429 Retry-After and rate-limit headers are measured without exposing credentials');check('STATIC-039 HTTP 429 error-body diagnostic', html.includes('429-error-reason=') && html.includes('retry-429-error-reason='), '429 body is reduced to sanitized error reason/message only');
...
async function main(){
  ...
  try {
    ...
    const ndlDefaultContract=await evalJS(`(()=>{const api=window.bookTrackerApiManagement;return {enabled:api.providers.ndl?.enabled,hasSearch:typeof api.adapters.ndl?.search==='function'}})()`); check('E2E-API-006L NDL直結検索の安全側デフォルト',ndlDefaultContract?.enabled===false&&ndlDefaultContract?.hasSearch===true,JSON.stringify(ndlDefaultContract));
