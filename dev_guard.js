...
async function main(){
  ...
  try {
    ...
    const ndlSearchContract=await evalJS(`(()=>{const api=window.bookTrackerApiManagement;const u=api?.buildNDLSruSearchUrl?.({title:'鬼滅の刃',limit:20,booksOnly:true})||'';const q=new URL(u).searchParams.get('query')||'';return {ok:q.includes('mediatype=books')&&new URL(u).searchParams.get('recordSchema')==='dcndl'&&new URL(u).searchParams.get('onlyBib')==='true'&&q.includes('title=')&&q.includes('鬼滅の刃')&&new URL(u).searchParams.get('dpid')==='iss-ndl-opac-bib',url:u,query:q}})()`);
