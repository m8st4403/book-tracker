...
## 調査結果のコピー／クリア範囲を統一
- 蔵書タブに個別結果コピー／クリア、蔵書タブ結果の一括コピー／クリアを追加。
- 設定タブに仕様・回帰チェック、登録計測、検索計測それぞれの個別コピー／クリアと、設定タブ結果の一括コピー／クリアを追加。
- シリーズ分類診断、書誌経路比較、ISBN直接登録準備診断、シリーズ再整理結果を個別に保持し、他の診断で上書きされても個別コピーできるよう整理。
- 「今回の調査結果をまとめてコピー／すべてクリア」を追加。共有も同じ全体レポートを使用。
- 「シリーズ・巻抜けチェック」は通常機能の位置を維持。
- 既存のファイル構成・単一CHANGELOG方針を維持。
