## v4.13.139
- NDLデータプロバイダ範囲診断を追加。シリーズ再整理ロジックは変更せず、同一ISBNをデータプロバイダ指定なし／国立国会図書館蔵書／全国書誌／新着書誌／デジタルコレクション／電子書籍・電子雑誌の各範囲で比較可能にした。

# v4.13.138

### NDLシリーズ候補取得診断を追加

- シリーズ情報が不足している通常蔵書を対象に、NDLタイトル検索で取得できる正規化候補をISBN単位で確認する診断を追加。
- ISBN一致候補、シリーズ名、巻数、検索語、上位候補、取得失敗を明示し、NDLの取得経路と正規化結果のどこで情報が失われているかを切り分け可能にした。
- 診断は1ISBNずつ順番に実行し、共通の診断操作ロックを使用。蔵書データは変更しない。
- 個別コピー／クリア、蔵書タブまとめコピー／クリア、全タブまとめコピーの対象へ追加。
...
- シリーズ再整理の診断に「現在値」「更新値」「実際に更新したか」をISBN単位で表示。
- 「更新候補あり」と「実際の変更」を区別し、変更されなかった理由を「更新不要（現在値と一致）」として表示。
- 従来の「シリーズ再整理経路診断（変更なし）」という誤解を招く見出しを「シリーズ再整理経路・更新判定診断」へ変更。
- API取得・採用判定・蔵書反映のどの段階で差異が発生しているかを確認できるようにした。

## 4.13.135

### 診断系操作の共通排他制御と結果分離

- シリーズ再整理、巻抜けチェック、シリーズ分類診断、ISBN/キーワード書誌比較、ISBN直接登録準備診断を共通の操作ロックへ接続。
- 実行中は他の診断・検索・データ操作をUI入口と関数入口の両方で拒否し、処理中表示を統一。
- 巻抜けチェック、シリーズ分類診断、書誌比較、登録準備診断、シリーズ再整理の結果表示領域を分離し、後続操作で前の結果を上書きしない。
- 蔵書タブのまとめコピー／クリアを5種類すべての診断結果へ拡張。
- 各診断結果に個別コピー／クリア操作を追加。
- 排他制御と診断結果分離を仕様ガードの回帰テストへ追加。

## 4.13.134
- シリーズ再整理時に、ISBN直接照会でシリーズ情報が得られない場合のみ、NDLのタイトル検索を追加するフォールバックを追加。取得結果はISBN一致を必須とし、シリーズ情報を持つ書誌だけを採用候補にする。
- 通常のISBN Resolverではこのフォールバックを実行せず、シリーズ再整理の用途に限定。
- NDLフォールバックの探索経路を診断結果に記録。
- 外伝・スピンオフ・短編集等を自動統合しないルール、既存のシリーズ分類ルールは変更しない。

## 4.13.133
- シリーズ再整理時のISBN Resolver呼び出しを同時実行3件から1件へ変更。NDL ISBN照会の同時アクセスによる一部ISBNの取得失敗を避け、v4.13.121～122で確認済みのISBNフォールバック経路を順序立てて実行する。
- シリーズ再整理の採用条件・外伝／スピンオフ等を自動統合しないルールは変更しない。
- 既存の蔵書データをシリーズ再整理以外で変更しない。

## 4.13.131

## 4.13.132
- シリーズ再整理の採用条件を修正。
- API Resolver が「採用可能」と判定したシリーズ情報について、シリーズ名と作品タイトルが一致しない出版シリーズ（例：レーベル名）でも、作品タイトル＋シリーズスコープで分類できるようにした。
- 巻数が両方取得できる場合はAPI取得巻と登録タイトル巻の一致を引き続き必須とし、外伝・スピンオフ等の自動統合ルールは変更しない。
- 既存のシリーズ情報・書籍データは診断実行だけでは変更しない。

- シリーズ再整理でResolverキャッシュを消去し、全API経路を確認する診断へ変更。API別シリーズ情報・探索経路を表示。
- 蔵書タブ結果コピーに対象範囲を明記。蔵書タブ以外の結果は混入させない。
- 既存蔵書データの自動変更ロジックは維持。

# v4.13.130

- シリーズ再整理の経路診断を追加。各ISBNについてAPI Resolverのシリーズ情報、信頼度、採用可否、対象外理由を表示できるようにした。
- 診断はシリーズ情報以外の蔵書データを変更せず、原因切り分けのための表示のみ行う。
- 既存のシリーズ再整理の保守的な自動更新条件は変更しない。

## v4.13.130
- 蔵書タブ・設定タブ・現在セッション全体の調査結果について、個別コピー／タブ単位コピー／全体コピーを整理。
- 調査結果の個別クリア／タブ単位クリア／全体クリアを追加。通常の蔵書データ・検索結果・設定値はクリア対象外。
- 結果クリア操作をRelease Gateの回帰対象へ追加。
- APP_VERSION / DEV_GUARD_VERSION / package.json / README / SPEC / APIキャッシュバスターを4.13.130へ統一。

## 4.13.128
- 蔵書統計の縮小5列表示がビューポート上端へ到達した時点で画面最上部に固定されるよう修正。
- 固定位置の基準をアプリヘッダーの文書上の高さからビューポート上端へ変更。
- APP_VERSION / DEV_GUARD_VERSION / package.json / README / SPEC / 設定表示 / APIキャッシュバスターの現行バージョンを4.13.128へ統一。
- 外部リリースガードにドキュメントを含むバージョン整合性と、五列表示の実ブラウザ固定位置検査を追加。
