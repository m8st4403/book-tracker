...
- GAP-005 設定：全設定ドメインのsave/read round-tripと保存失敗時ロールバックをGate化。
- GAP-006 カレンダー：一時表示フィルターと保存済み初期値の区別、追加失敗時ロールバックをGate化。
- GAP-009 通知：今日～7日後の境界、通知OFF、不正日付をGate化。
- GAP-010 ICS：発売日をVALUE=DATEで出力し、RFC 5545値エスケープと安定UIDをGate化。

### 検証基盤側の追加確認

設定保存失敗時にメモリだけ戻してUIが新設定のまま残るケースを防ぐため、最後の永続スナップショットへ戻した後 `loadSettingsUI()` でUIも再同期する契約を追加した。
